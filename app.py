...
if sport == "Home":
    ...
    # Use Streamlit's page switch (works when pages/1_🏈_NFL.py exists)
    try:
        st.switch_page("1_🏈_NFL")
    except Exception:
        # fallback: show friendly error
        st.error("Unable to switch to NFL page. Make sure pages/1_🏈_NFL.py exists.")

elif sport == "🏀 NBA":
    try:
        st.switch_page("2_🏀_NBA")
    except Exception:
        st.error("Unable to switch to NBA page. Make sure pages/2_🏀_NBA.py exists.")
