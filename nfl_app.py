...
from betting_analyzer import render_betting_tab  # Betting tab import
...
@@ -69,134 +49,128 @@ def load_nextgen_data(season, stat_type):
...
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs([
    ...
    def get_leaders(df, pos_codes, sort_option):
        ...
        if sort_option == "Touchdowns":
            ...
            sort_col = 'Total Yards'
        else:
            sort_col = 'Fantasy Pts'
...
@@ -205,247 +179,130 @@ def get_defense_stats(stats_df, position_codes):
                    if td_cols:
    ...
    # TAB 5: NEXTGEN STATS
            stat_col = 'passing_yards' if consistency_position=="QB" and 'passing_yards' in pos_df.columns else 'fantasy_points_ppr'
            player_consistency = pos_df.groupby('player_display_name').agg({stat_col: ['mean', 'std', 'count', 'min', 'max']}).reset_index()
            player_consistency.columns = ['Player', 'Avg', 'StdDev', 'Games', 'Min', 'Max']
            player_consistency['CV'] = (player_consistency['StdDev'] / player_consistency['Avg']) * 100
            max_cv = player_consistency['CV'].max()
            player_consistency['Consistency Rating'] = 100 - ((player_consistency['CV'] / max_cv) * 100)
            ...
            with col4:
                ...
                st.metric("Highest Ceiling", highest_ceiling['Player'], f"{highest_ceiling['Max']:.1f}")
            st.subheader(f"🎯 {consistency_position} Consistency Rankings")
            st.dataframe(player_consistency[['Player', 'Avg', 'Consistency Rating', 'Min', 'Max', 'Games']].sort_values('Consistency Rating', ascending=False).head(20), use_container_width=True, hide_index=True)
    ...
    with tab5:
        ...
        if nextgen_df.empty:
            ...
            if stat_type == "rushing" and 'rush_yards' in nextgen_df.columns:
                ...
                st.dataframe(top_df[['player_display_name','team_abbr','rush_yards','rush_attempts']], use_container_width=True, hide_index=True)
                ...
                st.dataframe(top_df[['player_display_name','team_abbr','receiving_yards','receptions']], use_container_width=True, hide_index=True)
                ...
                st.dataframe(top_df[['player_display_name','team_abbr','pass_yards','completions','attempts']], use_container_width=True, hide_index=True)
